...
S_API void* S_CALLTYPE SteamInternal_FindOrCreateUserInterface(HSteamUser hSteamUser, const char *pszVersion)
{
	if (pszVersion != nullptr)
	{
		...
		{
			void* pInterface = SteamClient_Client->GetISteamGenericInterface(hSteamUser, hPipe, pszVersion);

			if (pInterface == nullptr)
				NotifyMissingInterface(hPipe, pszVersion);
...
S_API void* S_CALLTYPE SteamInternal_FindOrCreateGameServerInterface(HSteamUser hSteamUser, const char *pszVersion)
{
	if (pszVersion != nullptr)
	{
		...
		{
			void* pInterface = SteamClient_Server->GetISteamGenericInterface(hSteamUser, hPipeServer, pszVersion);

			if (pInterface == nullptr)
				NotifyMissingInterface(hPipeServer, pszVersion);
