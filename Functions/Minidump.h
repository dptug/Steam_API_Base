CWin32MiniDump* Win32MiniDump = nullptr;
...
void LoadBreakpad(HMODULE hSteamclient)
{
	...
	{
		WriteColoredText(FOREGROUND_RED | FOREGROUND_INTENSITY, 7, "[Steam_API_Base] LoadBreakpad --> SteamAPI_UseBreakpadCrashHandler has not been called!\r\n");
		return;
	}
...
S_API void S_CALLTYPE SteamAPI_SetMiniDumpComment(const char *pchMsg)
{
	...
	{
		if (Win32MiniDump->InitStatus() == true)
		{
			Win32MiniDump->ClearComment();

			const int GetWCharSize = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, pchMsg, -1, nullptr, 0);

			if (GetWCharSize == 0)
			{
				WriteColoredText(FOREGROUND_RED | FOREGROUND_INTENSITY, 7, "[Steam_API_Base] SteamAPI_SetMiniDumpComment --> MultiByteToWideChar Failed!\r\n");
				return;
			}

			wchar_t* szComment = new wchar_t[GetWCharSize]();

			const int ConvertToWChar = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, pchMsg, -1, szComment, GetWCharSize);

			if (ConvertToWChar == 0)
			{
				WriteColoredText(FOREGROUND_RED | FOREGROUND_INTENSITY, 7, "[Steam_API_Base] SteamAPI_SetMiniDumpComment --> MultiByteToWideChar Failed! (2)\r\n");

				delete[]szComment;
				return;
			}

			Win32MiniDump->SetComment(szComment);
...
S_API void S_CALLTYPE SteamAPI_WriteMiniDump(uint32 uStructuredExceptionCode, void* pvExceptionInfo, uint32 uBuildID)
{
	...
	if (pvExceptionInfo != nullptr)
	{
		if (Win32MiniDump->InitStatus() == true)
		{
			Win32MiniDump->WriteUsingExceptionInfo(uStructuredExceptionCode, (_EXCEPTION_POINTERS*)pvExceptionInfo);
			Win32MiniDump->ClearComment();
		}
