...
{
	if (UseManualDispatch == 3) //Shutdown function was called
...
S_API void S_CALLTYPE SteamAPI_RunCallbacks()
{
	...
	//Steam006 note: The above is incorrect, the original dll only calls SteamAPI_ReleaseCurrentThreadMemory (Steam_ReleaseThreadLocalMemory) in SteamAPI_RunCallbacks when the pipe is invalid.
	...
	AcquireSRWLockExclusive(&CallbackLock);

	Steam_RunCallbacks(hPipe, false);

	ReleaseSRWLockExclusive(&CallbackLock);
...
S_API void S_CALLTYPE Steam_RunCallbacks(HSteamPipe hSteamPipe, bool bGameServerCallbacks)
{
	...
	{
		WriteColoredText(FOREGROUND_RED | FOREGROUND_INTENSITY, 7, "[Steam_API_Base] Steam_RunCallbacks --> Callback is already running!\r\n");
	}
